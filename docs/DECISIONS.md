...
## D38 — Phase 10b-2: the world's state in Firestore, behind a store seam

**Decided:** `world_server.py`'s in-memory `STATE` singleton becomes one of two backends behind a small interface in the new `world_store.py` (`get_state()`, `propose()`, `get_log()`, `reset()`). `world.py`'s `WorldState` / `reactive_filter` / `apply` are reused untouched - only *where the two positions live* changes.

- **`InMemoryWorldStore`** - one `WorldState` in this process's memory. **The default**: `python world_server.py`, the whole test suite, and any single-process local run stay on it, with no `google-cloud-firestore` import at all. Behaviorally identical to the pre-D38 code (asserted by a test that replays a move sequence against a raw `world.py` `WorldState` and checks positions match at every step).

- **`FirestoreWorldStore`** (`--firestore`) - the positions live in one document, `world/current`: `{a_position, b_position, log: [...], reset_at}`. A Cloud Run **Service** can run several instances and recycle them; a process-memory singleton doesn't survive that, and two robots on two instances would each act on a stale world - which defeats `reactive_filter`, the one thing the world exists to guarantee (PLAN.md §4's layer split). `propose()` is a `@firestore.transactional` read → `reactive_filter`/`apply` → write-back; Firestore retries on contention, so a second robot's proposal automatically re-evaluates against the first's *committed* move. `get_observation` stays a plain read (it's already range-gated per D26 - the sensor limit is unchanged, and the transaction's full-ground-truth read is the *reactive* layer, not something the robot sees).

**Doc model:** one fixed `world/current` document, not `episodes/{id}`. Two robot processes both read/write a well-known path with no episode-id to coordinate on - the same shape as "both connect to one `world_server.py`" today. `log` is an array field on that doc (an episode is ~20-40 entries, a few KB, far under Firestore's 1 MB cap; one read for `get_log()`), not a subcollection.

**Episode lifecycle:** the doc persists across server restarts now, so "fresh episode" becomes explicit. Three ways in: lazy-create (`get_observation` on a missing doc inits it at start positions), a new `reset_world()` MCP tool (the "new episode" trigger a control UI would also call), and `world_server.py --reset` (resets on boot - a local convenience matching "restart = fresh episode"; **not** for a multi-instance deploy, every instance would wipe the shared world).
