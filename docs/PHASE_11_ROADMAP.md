...
|---|---|
| `src/loop_world.py` (new in 11a, done) | Loop positions (mod `L`), `lane` field, two corridor spans, directional-lane model, `robots: dict[str, RobotState]` (not `.a`/`.b`). `reactive_filter` / `apply` → list passes generalizing the two safety invariants. Per-corridor `CorridorContest` (retired once both sides pass, so the same corridor re-negotiates fresh) replaces one whole-episode `priority`. Life/urgency/death bookkeeping; `survival_result()` reports ticks-alive per robot - no "winner" concept anywhere (D48). No `reached_target` termination - runs for a fixed tick budget. |
| `src/loop_scenarios.py` (new in 11a, done) | `RobotSpawn`/`LoopConfig`: `robots: tuple[RobotSpawn, ...]`, each `{name, corner, direction, urgency, situation}` (urgency = drain rate). Validated at construction (max 8, no reused `(corner, direction)` slot, no reused name). `duel`/`standoff` starter configs; `crowd` (11b) not built yet. "Who goes first" is computed live per-encounter (closer to death), not stored. `for_robot()` mirrors `for_side()`'s hidden-information discipline - not used until 11c makes robots real processes again. A wholly new file, not additions to `scenarios.py` - same reasoning as `loop_world.py` vs. `world.py`. |
| `src/loop_observation.py` (new in 11a, done) | A loop-aware composer, alongside (not inside) `observation.py`: single `other_*` → a list of sensed others (ahead / behind / genuinely opposing me toward a shared corridor). Drops "approaching from the opposite end" phrasing (false on a loop). Unlike `observation.py`, imports `loop_world` directly for its geometry (mod-`L` wraparound, `next_corridor`) rather than re-deriving it - the loop's arithmetic is real enough that duplicating it independently risked drift; the linear grid's `abs(position - entrance)` never had that risk. |
| `loop_simulate.py` (new in 11a, done) | Continuous run over a tick budget (default 40; the roadmap's own "real run" is 280); survival readout (alive/died-at-tick per robot) instead of `simulate.py`'s 6-tuple tick dump. `run_loop_episode()` recomposes every robot's observation fresh each tick (D24's discipline) before calling `loop_world.step()`. Live-verified on first run: `duel` reproduces the early-sensing crossing exactly as predicted (D48's simulation check); `standoff` deadlocks and both sides die exactly as PHASE_11_ROADMAP.md's own hypothesis states. |
